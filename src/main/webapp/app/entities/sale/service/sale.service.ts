...
type RestOf<T extends ISale | NewSale> = Omit<T, 'soldDate'> & {
...
export class SaleService {
  ...
  protected convertDateFromClient<T extends ISale | NewSale | PartialUpdateSale>(sale: T): RestOf<T> {
    return {
      ...sale,
      soldDate: sale.soldDate?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restSale: RestSale): ISale {
    return {
      ...restSale,
      soldDate: restSale.soldDate ? dayjs(restSale.soldDate) : undefined,
    };
  }
  ...
  searchByParam(name?: string, startDate?: string, endDate?: string): Observable<EntityArrayResponseType> {
    ...
    return this.http.get<ISale[]>(`${this.resourceUrl}/search`, { params: options, observe: 'response' });
  }
}
